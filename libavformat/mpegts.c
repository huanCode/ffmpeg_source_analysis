...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    p_end = section + section_len - 4;	//crc32 field前
    p     = section;
    if (parse_section_header(h, &p, p_end) < 0)
        return;
    if (h->tid != PAT_TID)
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    int afc = (packet[3] >> 4) & 3;	//afc:adaptation field control
    ...
    int is_discontinuity =	has_adaptation &&
						...
						discontinuity_indicator; /* and discontinuity indicated */

    /* continuity check (currently not used) */
	//随着每一个具有相同的PID的传送流分组而增加，当它达到最大值（15）后又回复到0。
	//如果adaptation_fiel_control为00或10，则continuity_counter不应增加。
	//00:not used
	//10:only adaptation,no payload
	//总结:continuity_counter对含有payload有效;当adaptation_fiel_control为00或10，
	//is_discontinuity应为1，表明当前payload不连续
    int continuity_counter = (packet[3] & 0xf);
	...
	const uint8_t *p_end = packet + TS_PACKET_SIZE;
    if (p >= p_end || !has_payload)
        return 0;

	int64_t pos = avio_tell(ts->stream->pb);
    if (pos >= 0) {
