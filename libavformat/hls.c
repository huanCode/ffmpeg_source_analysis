...
struct playlist {
    ...
    int64_t target_duration;	//#EXT-X-TARGETDURATION:time
    int start_seq_no;			//#EXT-X-MEDIA-SEQUENCE:number
    int n_segments;				//表示ts的个数
    struct segment **segments;	//表示ts的数组
...
typedef struct HLSContext {
    ...
    int n_playlists;	//playlists数组个数
    struct playlist **playlists;//数组
    int n_renditions;
    struct rendition **renditions;
...
//如果pls已经存在了，则创建playlist加入HLSContext中的playlist数组
